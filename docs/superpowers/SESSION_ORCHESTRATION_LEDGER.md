...
| 2026-07-25 | Close RH seed dashboard follow-up: made the demo RH fixture idempotently completed for onboarding and validated real `/dashboard` desktop/mobile smoke without redirecting to `/onboarding-rh`. | `src/lib/db/seed.ts`, `docs/deploy-vps-hostinger.md`, RH seed dashboard smoke report/screenshots under `C:\Users\user\Codex\2026-07-25\uniher-rh-seed-dashboard-smoke` | hold |
...
- Post-push visual smoke passed after the mobile scroll fix. Production deploy still requires real `JWT_SECRET`/`JWT_REFRESH_SECRET`; the seeded RH dashboard follow-up now passes with completed onboarding on `rh.visual@eduardaeyurimarketingltda.com.br`.
