...
- Evidence SHAs are deliberately split by gate: visual regression ran on `f87c5fd`; the integrated E2E matrix ran on `908357f`; the complete unit/build gate ran on `006770f`. No later SHA is inferred for an earlier execution.
...
- Integrated Wave 4 matrix on `908357f`: `community-feed`, `community-feed-ui`, `mobile-shell`, `privacy-wave-1-1`, and `seguranca`: **PASS, 65/65**, followed by a second **PASS, 65/65** after the mobile external-host guard, using **3 workers** and **zero retries**. Both receipts belong to the branch state at `908357f`, before `006770f`; no second matrix SHA is claimed.
